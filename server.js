...
async function installPackage() {
    try {
        let package = JSON.parse(fs.readFileSync('package.json'))['dependencies']
        
        for (let i = 0; i < mInstall.length; i++) {
            if (package[mInstall[i]] == null) {
                await installAsync(mInstall[i])
            }
        }
    } catch (error) {}
}

async function installAsync(package) {
    try {
        let tool = exec('npm install '+package)
